...
import Implicits.Homework.SuperVipCollections4s._
import Implicits.Homework.SuperVipCollections4s.syntax._
import ImplicitsHomeworkSpec.TestValue
...
class ImplicitsHomeworkSpec extends AnyFreeSpec with Matchers {
  ...
  "MutableBoundedCache" - {
    ...
    "when enough capacity should store values" in {
      val cache = new MutableBoundedCache[TestValue, TestValue](
        maxSizeScore = Int.MaxValue
      )
    ...
    "when not enough capacity should evict first inserted values" in {
      val cache =
        new MutableBoundedCache[TestValue, TestValue](maxSizeScore = 12)
  ...
  "Iterate2 instances provided" - {
    "for Map" in {
      assertCompiles(
        """import Implicits.Homework.SuperVipCollections4s._
          |import Implicits.Homework.SuperVipCollections4s.instances._
          |
  ...
  "TwitCache" - {
    import Implicits.Homework.MyTwitter._
...
private object ImplicitsHomeworkSpec {
  ...
  object TestValue {
    implicit val testValueGetSizeScore: GetSizeScore[TestValue] =
      (value: TestValue) => value.score
  }
}
